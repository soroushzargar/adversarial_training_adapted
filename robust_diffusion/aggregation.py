...
from robust_diffusion.helper.utils import sparse_tensor_to_tuple, tuple_to_sparse_tensor
...
def dense_soft_median(adj, x, temperature: float = 1.0):
    n, c = x.shape
    device = x.device

    lin_n = torch.arange(n, device=device)
    lin_c = torch.arange(c, device=device)

    weights = torch.abs(adj)
    sign = torch.sign(adj)
    sign[sign == 0] = 1  # TODO: only for debugging

    x_signed = sign[..., None] * x

    with torch.no_grad():
        sort_idx = x_signed.argsort(1)

        weights_cumsum = weights[lin_n[:, None, None],  sort_idx].cumsum(1)
        median_idx = (weights_cumsum < weights_cumsum[:, -1:, :] / 2).sum(1)

        median_idx = sort_idx[lin_n[:, None], median_idx, lin_c[None, :]]

    median = x_signed[lin_n[:, None], median_idx, lin_c]
    distances = torch.norm(x_signed - median[:, None], dim=-1) / pow(c, 1 / 2)

    soft_weights = weights * F.softmax(-distances / temperature, dim=1)
    norm = weights.sum(1, keepdims=True) / soft_weights.sum(1, keepdims=True)
    soft_weights = sign * norm * soft_weights

    return soft_weights @ x
